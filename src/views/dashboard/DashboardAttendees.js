...
const DashboardAttendees = () => {
  ...
  const {
    ...
    pagination,
    attendees,
  } = useContext(AttendeeProvider.Context);
      ...
      <CRow>
        <CCol>
          <CCard style={styles.cardStyle}>
            <CCardHeader style={styles.noBorder}>
              <CRow className="mt-4">
                <CCol md="5">
                  ...
                  <b>{attendees.length ? pagination.from : ""}</b>-
                  <b>{attendees.length ? pagination.to : ""}</b> of{" "}
                  <b>{attendees.length ? pagination.total : ""}</b> Results
                </CCol>
            ...
            <CCardBody>
              <table
                className="table table-hover  mb-2 d-none d-sm-table "
                          ...
                          <td>
                            <div className="clearfix">
                              <div className="float-left">
                                <a
                                  href={"/programmes/attendees/user/" + data.id}
                                >
                                  <span>
              ...
              {!attendees.length && fetchingFailMsg !== null && (
                <h3 className="mb-4">{fetchingFailMsg}</h3>
              )}

              {!attendees.length && fetching && (
                <h3 className="mb-4">Loading..</h3>
              )}

              {attendees.length ? (
                <Paginations context={AttendeeProvider.Context} />
              ) : (
                ""
              )}
