...
const Provider = (props) => {
  ...
  const fetchAttendeeByIdApi = (id) => {
    ...
    //alert("Hi there man")
    axios
      .get(API_URL + "programmes/attendees/user/" + id)
      .then((response) => {
        ...
        console.log(
          ...
          response.data.data.data[0]["refs"][0]["name"]
        );
      })
      .catch((error) => {
        setErrors([]);
        ...
        error.response !== undefined
          ? setFetchingFailMsg("No attendeed found")
  ...
  const fetchAttendeesByProgrammeIdApi = (id) => {
    ...
    axios
      .get(API_URL + "programmes/attendees/" + id)
      .then((response) => {
  ...
  const searchAttendeesByPhraseApi = () => {
    ...
    axios
      .get(API_URL + "programmes/attendees/search/" + searchPhrase)
      .then((response) => {
        setAttendees((prevArticle) => {
          return [...prevArticle, ...response.data.data.data.data];
        });
